#!/bin/bash -e
##########################################################################
# Creator: Tiziano Bevilacqua - 2022/06/08 based on previous script from Janos Karancsi
#   This is a Script that creates jobs to run PhaseIPixelNtuplizer on Slurm.
#   Make sure to use this on the lxplus in a directory accessible outside!
#   Jobs are submitted via the sbatch command - some experience helps
#   The script doesn't submit anything but
#   produces other scripts in the task directory that you can source
#   Feel free to try it!
#
#   make sure that you edit USERDIR first! (see tutorial)
#
# tutorial:
...
#   eg:
#   cmsenv      #in any CMSSW release area (CMSSW_*/src) here is assumed CMSSW_12_3_4_patch3
...
#   dasgoclient -query="file dataset=/ZeroBias/Run2022A-v1/RAW" | sort | grep "352/90" > file_list.txt [TimingScan2022]
#
# Step 1 - Create - Slurm.sh [TaskDir] -create [InputFile] [JobScript] [CMSSW_Version] [GlobalTag] [outdir] [python config] [datatier]
# 
#   grid-proxy-init --voms cms -valid 120:00
#   <Grid pass phrase>
#   source test/scripts/Slurm.sh My_test_task -create test_input.txt slurm_jobscript.sh CMSSW_12_3_4_patch3 GlobalTag outdir python config
#
# 
# Step 2 - Submit - Slurm.sh [TaskDir] -submit
#
#   source test/scripts/Batch.sh v3735_INC_SPL1_ns11_710pre1_MB_GENSIM_test -submit 
#   > source My_test_task/submit.sh
#
#
# Step 3 - Missing * - Slurm.sh [TaskDir] -missing
#
#   source test/scripts/Slurm.sh My_test_task -missing 
#   > 21,105,165,345,524,
#
#
# Step 4 - Resubmit * - Slurm.sh [TaskDir] -resubmit [list]
#
#   source test/scripts/Slurm.sh My_test_task -resubmit 21,105,165,345,524
#   > source My_test_task/resub.sh
# 
#
# Step 5 - Resubmit Missing - Slurm.sh [TaskDir] -resubmit_missing
#   * Combines the above two automatically
#
#   source test/scripts/Slurm.sh My_test_task resubmit_missing
#   > source My_test_task/resub.sh
#
#
# Step 6  - Delete - Slurm.sh [TaskDir] -delete
#   - Or you can simply delete the directory now
#
#   source test/scripts/Slurm.sh delete My_test_task
#
##########################################################################
...
if [ "$OPT" == "-create" ]; then
	...
	echo "Input file does not exist"
        ...
        echo "Python config:   "${8}
        echo "Data Tier:       "${9}
        echo
        cd $TASKDIR
        mkdir STDOUT
        chmod 666 STDOUT
        sed = input.txt | sed 'N;s;\n; ;' | awk '{ printf "%.4d %s\n", $1, $2 }' | awk '{ print "sbatch --job-name='$TASKDIR'_"$1" -o /work/%u/test/.slurm/%x_%A_"$1".out -e /work/%u/test/.slurm/%x_%A_"$1".err slurm_jobscript.sh '$TASKDIR'_JOB"$1" '$5' '$6' "$1" "$2" '$USERDIR'/'$1' '$8' '$9'"}' > alljobs.sh
        cd -
        echo "Task Created"
        echo "--------------------------------------------------------------------------------"
    fi
    ...
    ( squeue -n `cat $TASKDIR/alljobs.sh | awk '{print $2}' | sed 's;--job-name=;;g;' | sed ':label1 ; N ; $! b label1;s;\\n;,;g'` > taskjobs ) > /dev/null
