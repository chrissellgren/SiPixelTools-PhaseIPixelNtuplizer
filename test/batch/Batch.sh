...
USERDIR_EOS="t/tbevilac/PhaseIPixelNtuplizer"
...
if [ ! -d $TASKDIR ]; then
	...
	NJOBS=`wc -l $TASKDIR/alljobs.csh | awk '{ print $1}'`
    fi
fi
...
if [ "$OPT" == "-create" ]; then
	...
	echo "Input file does not exist"
        ...
        sed "s;outdir;"$TASKDIR";;s;userdir;"$USERDIR_EOS";" $5 > $TASKDIR/jobscript.sh
        chmod 777 $TASKDIR/jobscript.sh
        ...
        mkdir STDOUT
        chmod 666 STDOUT
        sed = input.txt | sed 'N;s;\n; ;' | awk '{ printf "%.4d %s\n", $1, $2 }' | awk '{ print "bsub -J '$TASKDIR'_JOB"$1" -oo STDOUT/JOB_"$1".log -q '$4' -L sh jobscript.sh '$6' '$7' "$1" "$2 }' > alljobs.sh
        cd -
        echo "Task Created"
        echo "--------------------------------------------------------------------------------"
    queue=${4}
